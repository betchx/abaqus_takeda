...
contains
  ...
  subroutine K_LINEAR_SPRING(&
    ...
    ! argument check
    if(JTYPE.ne.1) call abort('UEL bugs about user element type id')
    ...
    case(1) ! Normal incrementation
      ...
      case (1:2)
        ...
        if(LFLAGS(4).eq.1) then
          call updeteStaticLP(AMATRX,RHS,ENERGY,SRESID,PROPS,U,DU)
        else
          call updateStatic(AMATRX,RHS,ENERGY,SRESID,PROPS,U)
        endif
      case (11:12)
        !Implicit Dynamic
        call updateDynamic(AMATRX, RHS, ENERGY, SRESID, PROPS, U, V, A)
      case default
